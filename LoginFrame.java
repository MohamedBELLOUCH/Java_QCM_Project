...
public class LoginFrame implements ActionListener{
	...
	JLabel footer = new JLabel("<HTML><U>S'inscrire</U></HTML>", SwingConstants.CENTER);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == login) {
			...
			HashMap<String,String> eleves = new HashMap<String,String>(); //HashMap contenant les ids (sous forme string ici) et les passwords correspondants
			...
			try {
				...
				FileInputStream fi = new FileInputStream("./eleves.ser");
				ObjectInputStream oi = new ObjectInputStream(fi);			
				
				while (i < nbEleves) {
					eleve = (Eleve) oi.readObject();
					eleves.put(eleve.login, eleve.password);
					i++;
				}
			...
			if (eleves.containsKey(id)) {
				if (eleves.get(id).equals(pswd)) {
					message.setForeground(new Color(0x3BB54A));
					message.setText("Login Successfully !");
					message.setIcon(check_icon);
				}
					
				else {
					message.setForeground(new Color(0xD7443E));
					message.setText("Mot de passe incorrect !");
					message.setIcon(wrong_icon);
					}
			}
			else {
				message.setForeground(new Color(0xD7443E));
				message.setText("Identifiant Introuvable!");
				message.setIcon(wrong_icon);
			}
